...
void RadioSetRxConfig( RadioModems_t modem, uint32_t bandwidth,
                         ...
                         bool iqInverted, bool rxContinuous )
{
    printf("BW=%d,DR=%d,CR=%d,IQ=%d,symbTO=%d\r\n",bandwidth,datarate,coderate,iqInverted,symbTimeout);
    //printf("modem=%d,bandwidth=%d,datarate=%d,coderate=%d,",modem,bandwidth,datarate,coderate);
    //printf("bandwidthAfc=%d,preambleLen=%d,symbTimeout=%d,fixLen=%d\r\n",bandwidthAfc,preambleLen,symbTimeout,fixLen);
    //printf("payloadLen=%d,crcOn=%d,freqHopOn=%d,hopPeriod=%d,iqInverted=%d,rxContinuous=%d\r\n",payloadLen,crcOn,freqHopOn,hopPeriod,iqInverted,rxContinuous);
...
void RadioRxBoosted( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
        TimerSetValue( &RxTimeoutTimer, timeout );
        TimerStart( &RxTimeoutTimer );
        printf("RadioRxBoosted-->RxTimeoutTimer Start=%d\r\n",timeout);
    }
...
void RadioOnRxTimeoutIrq( void )
{
    ...
    {
        printf("SW-Timer-->RadioOnRxTimeoutIrq-->RxTimeoutTimer\r\n");
